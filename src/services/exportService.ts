...
async function exportAsXlsx(
  ...
  XLSX.utils.book_append_sheet(wb, ws, 'Expenza Expense Report');

  const base64Content = XLSX.write(wb, { type: 'base64', bookType: 'xlsx' });
  const filename = `Expenza-Expense-Report-${dateStamp}.xlsx`;
...
async function exportAsPdf(
    ...
    <html>
      ...
      <body>
        <div class="header-box">
          <div>
            <div class="brand-pre">Expenza</div>
        ...
        <div class="footer">
          <div>Generated by Expenza — 100% On-Device Local Finance</div>
  ...
  try {
    ...
    const filename = `Expenza-Expense-Report-${dateStamp}.pdf`;
    const baseDir = FileSystem.documentDirectory || FileSystem.cacheDirectory;
...
async function exportAsJson(
  ...
  const filename = `Expenza-Expense-Report-${dateStamp}.json`;
  const baseDir = FileSystem.documentDirectory || FileSystem.cacheDirectory;
  const fileUri = `${baseDir}${filename}`;

...
async function exportAsXlsx(
  ...
  const filename = `Expense-Report-${dateStamp}.xlsx`;
...
async function exportAsPdf(
    ...
    <html>
      ...
      <body>
        <div class="header-box">
          <div>
            <div class="brand-pre">Expense Tracker</div>
        ...
        <div class="footer">
          <div>Generated by ExpenseFlow — 100% On-Device Local Finance</div>
  ...
  try {
    ...
    const filename = `Expense-Report-${dateStamp}.pdf`;
    const baseDir = FileSystem.documentDirectory || FileSystem.cacheDirectory;
...
async function exportAsJson(
  ...
  const filename = `Expense-Report-${dateStamp}.json`;
  const baseDir = FileSystem.documentDirectory || FileSystem.cacheDirectory;
  const fileUri = `${baseDir}${filename}`;
